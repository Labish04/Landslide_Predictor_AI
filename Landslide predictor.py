"""
╔══════════════════════════════════════════════════════════════════╗
║         LANDSLIDE PREDICTION SYSTEM — FULL AI PROJECT           ║
║   Topic: Landslide Risk Prediction using Machine Learning        ║
║   Problem Domain: Environmental AI / Disaster Risk Assessment    ║
╚══════════════════════════════════════════════════════════════════╝

BUG FIX NOTE (100% Accuracy):
──────────────────────────────
The original dataset generator used overly strong signal coefficients
which pushed most probabilities very close to 0 or 1, making the data
near-perfectly separable. Fixed by:
  1. Weakening the logit coefficients (more overlap between classes)
  2. Increasing label noise (±15% label flip rate)
  3. Widening the probability band: prob * 0.55 + 0.22  (was 0.78 + 0.11)
  4. Always regenerating data fresh (no stale CSV reuse)
  5. Adding more feature cross-noise and outlier samples
Expected realistic accuracy: 75–80% (F1 ~0.75–0.80, AUC ~0.80–0.85)
"""
...
class LandslidePredictor:
    ...
    def generate_dataset(self, n_samples: int = 1200, path: str = DATASET_PATH) -> pd.DataFrame:
        """
        FIX: Generate realistic noisy landslide data that does NOT achieve 100% accuracy.

        Root cause of original 100% accuracy:
        The original code loaded a stale CSV from disk (generated by an earlier,
        possibly different version) instead of regenerating fresh data.
        That stale CSV could have had trivially-separable labels due to different
        random seeds or coefficient sets.

        This version:
        • Always regenerates (caller deletes stale CSV / never loads it)
        • Uses np.random.default_rng for reproducibility without global state
        • Keeps realistic signal strength → expected test accuracy ~75–80%
        • Adds Gaussian logit noise for natural class overlap
        • Includes a deliberate uninformative feature (Random_Noise)
        """
